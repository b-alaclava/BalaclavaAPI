package com.example.Plugins.LunatikTitheFarm;


import com.example.BalaclavaAPI.InteractionUtility.InventoryUtility;
import com.example.BalaclavaAPI.Utility.ClickObject;
import com.example.BalaclavaAPI.Utility.ClickWidget;
import com.example.BalaclavaAPI.Utility.DoAction;
import com.example.BalaclavaAPI.Utility.Walk;
import com.example.EthanApi.Collections.Inventory;
import com.example.EthanApi.Collections.TileObjects;
import com.example.EthanApi.Collections.Widgets;
import com.example.EthanApi.Collections.query.ItemQuery;
...
import static com.example.EthanApi.Collections.query.TileObjectQuery.getObjectComposition;
import static com.example.EthanApi.EthanApi.sendClientMessage;
import static com.example.EthanApi.EthanApi.stopPlugin;
...
public class AutoTitheFarmPlugin extends Plugin {
    ...
    private void initValues() {
        ...
        clientThread.invoke(() -> Inventory.search().withId(ItemID.GRICOLLERS_CAN).first().ifPresent(itm -> InventoryUtility.use(itm, "Check")));
    ...
    private void openFarmDoor() {
        ...
        TileObjects.search().nameContains("Farm door").first().ifPresent(obj -> ClickObject.objectAction(obj, "Open"));
    ...
    private void doAction(List<TileObject> collection) {
        ...
        ClickObject.objectAction(patch, "Water", "Harvest");
        actionDelayHandler.setWaitForAction(true);
    ...
    private void useItemOnObject(Widget widget, TileObject tileObject) {
        ...
        optionalWidget.ifPresent(itm -> {
            ClickWidget.widgetOnObject(itm, tileObject);
        });
        actionDelayHandler.setWaitForAction(true);
    }
    ...
    private void openHerbBox() {
        Consumer<? super Widget> actionPredicate = itm -> InventoryUtility.use(itm, "Bank-all");
    ...
    private void handleMinigame() {
        ...
        final boolean isPlayerMoving = EthanApi.isMoving() || client.getLocalPlayer().getAnimation() != -1;
        ...
        if (gricollersFertiliser.isPresent()) {
            InventoryUtility.use(gricollersFertiliser.get(), "Drop");
            return;
        }

        if (runEnergy > 0 && !isRunEnabled) {
            Widgets.search().withAction("Toggle run").first().ifPresent(run->{
                ClickWidget.widgetAction(run,"Toggle run");
            });
            return;
        }
        ...
        if (startingNewRun()) {
            ...
            if (getRegularCansCount() != -1 && getRegularCansCount() < (getAllRegularWateringCan().result().size() * REGULAR_WATERING_CAN_MAX_CHARGES)) {
                ...
                regularWateringCansToRefill.forEach(itm -> {
                    ClickWidget.widgetOnObject(itm, waterBarrel.orElse(null));
                });
                return;
            }

            // whether to trigger earlier if running.
            int runEnergyExtraDeviation = EthanApi.isMoving() ? runEnergyDeviation + 5 : runEnergyDeviation;
            ...
            if (config.useStaminaPot() && staminaPot.isPresent() && runEnergy < 70 && client.getVarbitValue(Varbits.RUN_SLOWED_DEPLETION_ACTIVE) == 0) {
                InventoryUtility.use(staminaPot.get(), "Drink");
    ...
    private void handleLobby() {
        ...
        if (getSeed() == null) {
            ...
            if (firstChatWindowId.isEmpty()) {
                TileObjects.search().withId(ObjectID.SEED_TABLE).first().ifPresent(obj -> ClickObject.objectAction(obj, "Search"));
                return;
            }

            int finalFirstChatOptionId = firstChatOptionId;
            Widgets.search().withId(14352385).first().ifPresent(wid->{
                ClickWidget.clickWidget(finalFirstChatOptionId,wid,MenuAction.WIDGET_CONTINUE,0);
